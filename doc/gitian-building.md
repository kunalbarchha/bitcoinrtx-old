...
*Setup instructions for a gitian build of Bitcoinrtx using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bitcoinrtx
...
- [Building Bitcoinrtx](#building-bitcoinrtx)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-bitcoinrtx-host/README.md).
...
Clone the git repositories for bitcoinrtx and gitian and then checkout the bitcoinrtx version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bitcoinrtx/bitcoinrtx
cd bitcoinrtx
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bitcoinrtx repository
...
To build Bitcoinrtx (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bitcoinrtx repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoinrtx/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/bitcoinrtx/bitcoinrtx
...
For example:
```bash
URL=https://github.com/laanwj/bitcoinrtx.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitcoinrtx=${COMMIT} --url bitcoinrtx=${URL} ../bitcoinrtx/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoinrtx=${COMMIT} --url bitcoinrtx=${URL} ../bitcoinrtx/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcoinrtx=${COMMIT} --url bitcoinrtx=${URL} ../bitcoinrtx/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoinrtx-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoinrtx-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bitcoinrtx-build.assert
```
...
[bitcoinrtx/gitian.sigs](https://github.com/bitcoinrtx/gitian.sigs/) repository, or if that's not possible create a pull
