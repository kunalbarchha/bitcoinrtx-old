...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars BITCOINRTXD and BITCOINRTXCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

BITCOINRTXD=${BITCOINRTXD:-${1}/bitcoinrtxd}
CLI=${BITCOINRTXCLI:-${1}/bitcoinrtx-cli}
...
$BITCOINRTXD -zapwallettxes=1 $B1ARGS &
B1PID=$!
$BITCOINRTXD -zapwallettxes=2 $B2ARGS &
B2PID=$!
