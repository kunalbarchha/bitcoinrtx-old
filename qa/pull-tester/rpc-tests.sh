...
export BITCOINRTXCLI=${BUILDDIR}/qa/pull-tester/run-bitcoinrtx-cli
export BITCOINRTXD=${REAL_BITCOINRTXD}
...
if [ "x${ENABLE_BITCOINRTXD}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and bitcoinrtxd must all be enabled"
fi
